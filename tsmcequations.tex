...
For $i=k\le j<l$, ($t_3=s_3\,;t_2>s_2$):
    % first summand
\begin{align*}
    % from above
    &=\frac{1}{2s_2+s_3}e^{-2\Omega(s_3,s_2)}\frac{1}{\lambda_{\alpha(t_2)}}e^{-\Omega(s_2,t_2)}
...
For $k<i\le j=l$ ($t_3<s_3; t_2=s_2$):
...
The other case that requires special consideration is $i<k=l=j$.
...
attention to the boundary cases, or the cases where $i=j$. (Update: for the
cases where $i=j$, $s_3$ and $s_2$ are $\E_{i,j}[s_3]$ and $\E_{i,j}[s_2]$
respectively --- shouldn't be a problem)
