...
$\gamma_i(t)$ and $\xi_{i,j}(t)$. We define 
...
\begin{equation}
    \tilde{\alpha}_j(t+1) = \frac{1}{s_{i+1}}e_j(y_{t+1})
...
\begin{equation}
    \tilde{\beta}_i(t) = \frac{1}{s_i}\sum_{j=1}^N
    A_{i,j}\tilde{\beta}_j(t+1)e_j(y_{t+1}).
\end{equation}
...
\begin{align}
    \begin{split}
        ...
        &=
        \frac{\tilde{\alpha}_i(t)\tilde{\beta}_i(t)\left(\prod_{j=1}^ts_j\right)^2}
        {\sum_{j=1}^N\tilde{\alpha}_j(t)\tilde{\beta}_j(t)\left(\prod_{j=1}^ts_j\right)^2} \\
        &=
...
\begin{align}
    \begin{split}
        \xi_{i,j}(t) &= 
                 ...
                 &= 
        \frac{\alpha_i(t)P_{i,j}e_j(t+1)\beta_j(t+1)}
        {\sum_{k=1}^N\sum_{l=1}^N\alpha_k(t)P_{k,l}e_l(t+1)\beta_l(t+1)} \\
                 &= 
        \frac{\tilde{\alpha}_i(t)P_{i,j}e_j(t+1)\tilde{\beta}_j(t+1)\left(\prod_{i=1}^ts_i\right)^2s_{i+1}}
        {\sum_{k=1}^N\sum_{l=1}^N\tilde{\alpha}_k(t)P_{k,l}e_l(t+1)\tilde{\beta}_l(t+1)\left(\prod_{i=1}^ts_i\right)^2s_{i+1}} \\
                 &= 
        \frac{\tilde{\alpha}_i(t)\,P_{i,j}\,e_j(t+1)\,\tilde{\beta}_j(t+1)}
        {\sum_{k=1}^N\sum_{l=1}^N\tilde{\alpha}_k(t)\,P_{k,l}\,e_l(t+1)\,\tilde{\beta}_l(t+1)}.\\
    \end{split}
\end{align}

% omegas and zetas, underflow. write up entire notes in book
